...
demo_rules(Ag, R,  _, FlagCN, InGoals, OutGoals ) :-
  ...
  ( agent_goal( Ag, G ) -> NewFirstPlan = [Abds, ( Cont :: Rest, UCRest), CN, HF, M]
...
demo_rules(Ag, R, _, FlagCN, InGoals, OutGoals ) :-
  ...
  (observable(Ag, G); for_testing_only(Ag, G)), Cont = c(T,L), T\=t, !,
...
abducible( Ag, G ) :- G=..[Ab|_], abd(Ag,Ab).
