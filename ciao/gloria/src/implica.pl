...
/************************************************ Ciao port: latent branches */
% The following predicates are referenced by case-analysis/case-splitting
% clauses that the engine does not reach in the current flow.  SWI-Prolog
% tolerated the undefined references at compile time (they only produced
% errors if ever executed); Ciao requires a definition.  These stubs fail,
% which is the same behaviour as never being reached.  Do NOT remove them
% unless the corresponding engine branch is completed and exercised.

builtin(_, _, _) :- fail.
valid_ineq(_) :- fail.
check_his(_, _, _, _, _) :- fail.
